...
- **Context-menu dismiss:** after an unchorded right-button release (a same-button release tap, settings change, or shutdown), `WinInjector` injects an `Esc` so the context menu the up would open is dismissed (see the right-button caveat under Behavior). Releasing a right lock because another mouse button was pressed is already chorded and does not inject `Esc`, avoiding an unrelated cancel action in the foreground app.
...
6. **Release the lock on session lock / fast-user-switch** (`WTSRegisterSessionNotification` -> `ReleaseAll`). The standalone reference app did this; if a button is locked when the session locks, it stays logically held until the next physical tap. Minor and self-healing.
